...
abstract contract Airdrop is Ownable, ReentrancyGuard {
  
  mapping(address => uint) public processedAirdropAmounts;
  IBEP20 public token;
  uint public currentAirdropAmount;
  uint public maxAirdropAmount = 48000000000000 * 10 ** 18;
  ...
  function initialize(address _signer, address _token, uint _amount) external virtual onlyOwner {
    _setupToken(_token);
